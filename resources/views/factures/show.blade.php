...
<head>
    ...
    <title>Facture de {{$factures->client}}</title>
    @vite('resources/css/app.css')
</head>
<body>
<h1 class="bg-red-800 text-white justify-center p-40 text-center text-4xl">Gestionnaire clients - factures</h1>
    <h2 class="text-3xl ml-20 mt-20 font-semibold">Facture de {{$factures->client}}</h2>
    <div class="flex justify-center items-center mt-20">
        <div class="bg-white rounded-lg shadow-2xl flex flex-col justify-center w-2/3 h-2/3 text-center border border-red-800">
            <p class="my-4 text-3xl">{{$factures->ref}}</p>
            <p class="text-lg my-4">{{$factures->titre}}</p>
            <p class="text-gray-700 my-4 text-lg">{{$factures->prix}}€</p>
            <p class="text-gray-700 my-4 text-lg">{{$factures->tva}}%</p>
            <p class="text-green-600 font-bold my-4 text-lg">{{$factures->total}}€</p>
            <p class="text-gray-700 my-4 text-lg">{{$factures->description}}</p>
        </div>
    </div>
